...
def execute_command(command, arguments, stdin=None):
    ...
    # check if the command is supported/allowed
    # ⚠️ IMPORTANT: prevents arbitrary command execution
    if command not in supported_command_names:
        raise CommandDisallowedError(f"Command \"{command}\" is not "
            "allowed.")
    ...
    # Run command in chroot in every enviornment except development
    ...
    if not IS_DEV:
        operation = ["chroot", f"--userspec={JAIL_UID}", JAIL_PATH]
    
    # add the user-input command to the list of arguments
    operation += [command]

    # some commands (like sort) don't expect arguments and will try to open a
    # file with the name [empty string] if called with an empty string
    # argument, so we only append arguments if they are NOT an empty string.
    if arguments:
